...
class MavicDriver:
    def init(self, webots_node, properties):
        ...
        self.__position_pub = self.__node.create_publisher(PointStamped, 'position', 1)
    ...
    def step(self):
        ...
        _, _, vertical = gps_values

        # Publish GPS position in Webots world-frame metres
        pt = PointStamped()
        pt.header.stamp = self.__node.get_clock().now().to_msg()
        pt.header.frame_id = 'world'
        pt.point.x = gps_values[0]
        pt.point.y = gps_values[1]
        pt.point.z = gps_values[2]
        self.__position_pub.publish(pt)
        ...
        if vertical > 0.2:
            # Calculate velocity (guard prevents divide-by-zero when flat)
            attitude_magnitude = abs(roll) + abs(pitch)
            if attitude_magnitude > 1e-6:
                velocity_x = (pitch / attitude_magnitude) * velocity
                velocity_y = -(roll / attitude_magnitude) * velocity
            else:
                velocity_x = 0.0
                velocity_y = 0.0
